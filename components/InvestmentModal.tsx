...
const InvestmentModal: React.FC<InvestmentModalProps> = ({ isOpen, onClose, onSave, investment, userId, t, lang }) => {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4">
            <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-2xl w-full max-w-md">
                <h2 className="text-2xl font-bold mb-6 text-gray-800 dark:text-gray-100">
                    {investment ? t('editInvestment', lang) : t('addInvestment', lang)}
                </h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t('investmentName', lang)}</label>
                    ...
                    <div>
                        ...
                        <select name="relatedProjectId" value={formData.relatedProjectId || ''} onChange={handleChange} className="mt-1 w-full p-2 border rounded-md dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                            <option value="">{t('noProject', lang)}</option>
