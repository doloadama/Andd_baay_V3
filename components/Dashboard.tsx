...
const Dashboard: React.FC<DashboardProps> = ({ user, t, lang, isHarvestMode, isDarkMode }) => {
    ...
    if (isHarvestMode) {
        ...
        return (
            <div className="space-y-8">
                 <h2 className="text-3xl font-bold text-amber-800 dark:text-amber-300">{t('projectsReadyForHarvest', lang)}</h2>
                 {harvestingProjects.length > 0 ? (
                        ...
                        {harvestingProjects.map(project => (
                            <div key={project.id} className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg hover:shadow-2xl transition-shadow duration-300 border-l-4 border-amber-500 flex flex-col justify-between">
                                ...
                                <div className="mt-6 border-t dark:border-gray-700 pt-4 flex flex-col space-y-2">
                                    <button onClick={() => handleOpenProductModal(project)} className="w-full flex items-center justify-center text-sm px-4 py-2 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 transition-colors"><ListPlus size={16} className="mr-2"/> {t('listProductFromHarvest', lang)}</button>
                                    <button onClick={() => handleOpenProjectModal(project)} className="w-full flex items-center justify-center text-sm px-4 py-2 bg-blue-500 text-white font-semibold rounded-lg hover:bg-blue-600 transition-colors"><TrendingUp size={16} className="mr-2"/> {t('updateYield', lang)}</button>
                                    <button onClick={() => handleMarkComplete(project.id)} className="w-full flex items-center justify-center text-sm px-4 py-2 bg-gray-200 text-gray-700 dark:bg-gray-700 dark:text-gray-200 font-semibold rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors"><CheckSquare size={16} className="mr-2"/> {t('markAsComplete', lang)}</button>
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="text-center bg-white dark:bg-gray-800 p-12 rounded-xl shadow-md">
                        <p className="text-lg text-gray-500 dark:text-gray-400">{t('noHarvestingProjects', lang)}</p>
                    </div>
                 )}
    ...
    return (
        <div className="space-y-8">
            <h2 className="text-3xl font-bold text-gray-800 dark:text-gray-100">{t('welcomeBack', lang, { name: user.name.split(' ')[0] })}</h2>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatCard title={t('activeProjects', lang)} value={projects.filter(p => p.status !== ProjectStatus.COMPLETED).length.toString()} icon={Briefcase} trend="+2 this week" trendColor="text-blue-500" />
                <StatCard title={t('totalRevenue', lang)} value={formatCurrency(totalIncome)} icon={TrendingUp} trend="+10.2%" trendColor="text-green-500"/>
                <StatCard title={t('totalExpenses', lang)} value={formatCurrency(totalExpenses)} icon={TrendingDown} trend="+5.1%" trendColor="text-red-500"/>
                <StatCard title={t('netProfit', lang)} value={formatCurrency(netProfit)} icon={DollarSign} trend="-1.5%" trendColor="text-red-500"/>
            </div>
                    ...
                    <div style={{ width: '100%', height: 300 }}>
                        <ResponsiveContainer>
                            <AreaChart data={financialChartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                                ...
                                <Area type="monotone" dataKey="income" stroke="#10B981" fillOpacity={1} fill="url(#colorIncome)" name={t('income', lang)} strokeWidth={2} />
                                <Area type="monotone" dataKey="expense" stroke="#EF4444" fillOpacity={1} fill="url(#colorExpense)" name={t('expense', lang)} strokeWidth={2} />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700/50 dark:text-gray-400">
                            <tr>
                                <th scope="col" className="px-6 py-3">{t('projectName', lang)}</th>
                                <th scope="col" className="px-6 py-3">{t('cropType', lang)}</th>
                                <th scope="col" className="px-6 py-3">{t('endDate', lang)}</th>
                                <th scope="col" className="px-6 py-3">{t('status', lang)}</th>
                                <th scope="col" className="px-6 py-3 text-right">{t('actions', lang)}</th>
                            </tr>
                        </thead>
                        <tbody>
