...
const Header: React.FC<HeaderProps> = ({ user, onLogout, t, lang, setLanguage, cart, setView, isHarvestMode, setIsHarvestMode, isDarkMode, setIsDarkMode }) => {
    ...
    return (
        <header className={`flex items-center justify-between p-4 border-b transition-colors duration-500 ${isHarvestMode ? 'bg-amber-500 border-amber-600' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 shadow-sm'}`}>
            <h1 className={`text-xl font-semibold ${isHarvestMode ? 'text-white' : 'text-gray-800 dark:text-white'}`}>{t('appName', lang)}</h1>
            <div className="flex items-center space-x-4">
                 <div className="flex items-center space-x-2">
                    <span className={`text-sm font-medium ${isHarvestMode ? 'text-white' : 'text-gray-600 dark:text-gray-300'}`}>{t('harvestMode', lang)}</span>
                    <button
                ...
                <div className="text-right">
                    ...
                    <p className={`text-sm capitalize ${isHarvestMode ? 'text-amber-100' : 'text-gray-500 dark:text-gray-400'}`}>{t(user.role.toLowerCase() as any, lang)}</p>
                </div>
                <button
                    onClick={onLogout}
