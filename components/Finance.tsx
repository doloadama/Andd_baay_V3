...
import { User, Transaction, ExpenseCategory } from '../types';
import { MOCK_TRANSACTIONS, MOCK_SITES, MOCK_PROJECTS } from '../constants';
...
const Finance: React.FC<FinanceProps> = ({ user, t, lang, isDarkMode }) => {
    ...
    const userSites = useMemo(() => MOCK_SITES.filter(s => s.farmerId === user.id), [user.id]);
    
    const projectsForSelectedSite = useMemo(() => {
        if (!filterSiteId) return [];
        return MOCK_PROJECTS.filter(p => p.siteId === parseInt(filterSiteId, 10));
    }, [filterSiteId]);
    ...
    const filteredTransactions = useMemo(() => {
        ...
        return transactions.filter(tx => {
            const siteMatch = tx.siteId === parseInt(filterSiteId, 10);
            if (!filterProjectId) {
                return siteMatch;
            }
            const projectMatch = tx.projectId === parseInt(filterProjectId, 10);
            return siteMatch && projectMatch;
        });
    }, [transactions, filterSiteId, filterProjectId]);
            ...
            <h2 className="text-3xl font-semibold text-gray-800 dark:text-gray-100 mb-6">{t('navFinance', lang)}</h2>
             ...
             <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md mb-8">
                <div className="flex flex-wrap items-center gap-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t('filterBySite', lang)}</label>
                        <select 
                            value={filterSiteId} 
                        ...
                        >
                            <option value="">{t('allSites', lang)}</option>
                            {userSites.map(site => <option key={site.id} value={site.id}>{site.name}</option>)}
                        </select>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t('filterByProject', lang)}</label>
                         <select 
                        ...
                        >
                            <option value="">{t('allProjects', lang)}</option>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
             ...
             <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 mb-8">
                <div className="lg:col-span-3 bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                    <h3 className="text-xl font-semibold text-gray-700 dark:text-gray-200 mb-4">{t('financialTrends', lang)}</h3>
                    <div style={{ width: '100%', height: 300 }}>
                        <ResponsiveContainer>
                            <LineChart data={financialChartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                                ...
                                <Line type="monotone" dataKey="income" name={t('income', lang)} stroke="#10B981" strokeWidth={2} />
                                <Line type="monotone" dataKey="expense" name={t('expense', lang)} stroke="#EF4444" strokeWidth={2} />
                            </LineChart>
                        </ResponsiveContainer>
                    </div>
                </div>
                <div className="lg:col-span-2 bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                    <h3 className="text-xl font-semibold text-gray-700 dark:text-gray-200 mb-4">{t('expenseBreakdown', lang)}</h3>
                    <div style={{ width: '100%', height: 300 }}>
                         <ResponsiveContainer>
                            <PieChart>
                                <Pie
                                    ...
                                    dataKey="value"
                                    nameKey="name"
                                    // Fix: The 'percent' property provided by Recharts Pie component can be undefined.
                                    // Added a check to ensure 'percent' is a number before performing arithmetic operations to prevent a TypeScript error.
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                         <thead className="bg-gray-50 dark:bg-gray-700">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{t('category', lang)}</th>
                                ...
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{t('relatedSite', lang)}</th>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{t('relatedProject', lang)}</th>
                        ...
                        <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            {filteredTransactions.map(tx => {
                                const site = MOCK_SITES.find(s => s.id === tx.siteId);
                                const project = MOCK_PROJECTS.find(p => p.id === tx.projectId);
                                return (
                                <tr key={tx.id}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{tx.category ? t(tx.category, lang) : 'N/A'}</td>
                     ...
                     {filteredTransactions.length === 0 && <p className="text-gray-500 dark:text-gray-400 text-center py-4">{t('noTransactions', lang)}</p>}
                </div>
            </div>
