...
const ImageEditor: React.FC<ImageEditorProps> = ({ t, lang }) => {
    ...
    return (
        <div>
            <h2 className="text-3xl font-semibold text-gray-800 dark:text-gray-100 mb-6">{t('navImageStudio')}</h2>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                ...
                >
                    {originalImage ? t('changeImage') : t('uploadImage')}
                </button>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="text-center">
                        <h3 className="font-semibold text-lg mb-2 dark:text-gray-200">{t('original')}</h3>
                        <div className="aspect-square bg-gray-100 dark:bg-gray-700 rounded-md flex items-center justify-center">
                            {originalImage ? <img src={originalImage} alt="Original" className="max-h-full max-w-full rounded-md" /> : <p className="text-gray-500 dark:text-gray-400">{t('noImageUploaded')}</p>}
                        </div>
                    </div>
                    <div className="text-center">
                        <h3 className="font-semibold text-lg mb-2 dark:text-gray-200">{t('edited')}</h3>
                        <div className="aspect-square bg-gray-100 dark:bg-gray-700 rounded-md flex items-center justify-center">
                            {loading && <p className="text-gray-500 dark:text-gray-400">{t('generating')}</p>}
                            {error && <p className="text-red-500">{error}</p>}
                            {editedImage && <img src={editedImage} alt="Edited" className="max-h-full max-w-full rounded-md" />}
                            {!loading && !error && !editedImage && <p className="text-gray-500 dark:text-gray-400">{t('editResultPlaceholder')}</p>}
                        </div>
                    </div>
                </div>
                ...
                {originalImage && (
                    <div className="mt-6">
                        <label htmlFor="prompt" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('editInstruction')}</label>
                        <textarea
                            id="prompt"
                            value={prompt}
                        ...
                        >
                            {loading ? t('processing') : t('generateEdit')}
                        </button>
                    </div>
                )}
            </div>
