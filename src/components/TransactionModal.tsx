...
// Fix: Add Site type for userSites prop
import { Transaction, Project, ExpenseCategory, Site } from '../types';
...
const TransactionModal: React.FC<TransactionModalProps> = ({ isOpen, onClose, onSave, transaction, userId, t, lang, userSites, projects }) => {
    const [formData, setFormData] = useState(initialFormData);
    
    const availableProjects = useMemo(() => {
        if (!formData.siteId) return [];
        return projects.filter(p => p.siteId === formData.siteId);
    }, [formData.siteId, projects]);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4">
            <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-2xl w-full max-w-md">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t('relatedSite')}</label>
